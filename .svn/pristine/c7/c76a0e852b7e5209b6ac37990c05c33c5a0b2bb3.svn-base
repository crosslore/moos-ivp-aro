...
bool CMOOSBridge::OnConnectToServer()
{
  ...
  if(m_nLocalUDPPort != 0)
    m_Comms.Notify("PMB_UDP_LISTEN", m_nLocalUDPPort);
  return(true);
}
...
bool CMOOSBridge::Configure()
{
    ...
    // May 11, 2012, mikerb change: m_nLocalUDPPort is an instance var, not local var.
    m_nLocalUDPPort = DEFAULT_UDP_PORT;
    if(m_MissionReader.GetConfigurationParam("UDPListen", m_nLocalUDPPort)) {
      //start the UDP listener
      m_UDPLink.Run(m_nLocalUDPPort);
      m_Comms.Notify("PMB_UDP_LISTEN", m_nLocalUDPPort);
    }
    else {
